 ...
 */
public class OldFuzzyGlobalKnowledgeRouter implements RoutingDecisionEngine{
    ...
    public OldFuzzyGlobalKnowledgeRouter(Settings s) {
    ...
    public OldFuzzyGlobalKnowledgeRouter(OldFuzzyGlobalKnowledgeRouter prototype) {
    ...
    public boolean shouldSendMessageToHost(Message m, DTNHost otherHost) {
        OldFuzzyGlobalKnowledgeRouter myPartner = getOtherFuzzyRouter(otherHost);
        if (m.getTo() == otherHost) {
            return true;
        }
        return this.getTOUValueFor(m.getTo()) < myPartner.getTOUValueFor(m.getTo());
    }
    ...
    public RoutingDecisionEngine replicate() {
        return new OldFuzzyGlobalKnowledgeRouter(this);
    }
    
    private OldFuzzyGlobalKnowledgeRouter getOtherFuzzyRouter(DTNHost host) {
        MessageRouter otherRouter = host.getRouter();
        assert otherRouter instanceof DecisionEngineRouter : "This router only works "
                + " with other routers of same type";

        return (OldFuzzyGlobalKnowledgeRouter) ((DecisionEngineRouter) otherRouter).getDecisionEngine();
    }
    /**
     * Change from fuzzy value to crisp
     * and compute utility value for host
     * @param host 
     */
    ...
    private double encounterVeryLowFunciton(DTNHost host) {
        if (getEncounterFor(host) <= contactDurationPoint[0]) return 1.0;
    ...
    private double encounterVeryHighFunciton(DTNHost host) {
        ...
        else if (getEncounterFor(host) >= contactDurationPoint[7]) return 1;
        else return 0;
    }

    private double intercontactVeryLowFunciton(DTNHost host) {
        if (getIntercontactFor(host) <= intercontactDurationPoint[0]) return 1.0;
    ...
    private double intercontactVeryHighFunciton(DTNHost host) {
        ...
        else if (getIntercontactFor(host) >= intercontactDurationPoint[7]) return 1;
        else return 0;
    }
